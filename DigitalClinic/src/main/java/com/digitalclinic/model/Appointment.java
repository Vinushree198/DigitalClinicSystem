...
public class Appointment {
    ...
    // ✅ FIXED computed property for JSP
    @Transient
    public boolean getCanBeCancelled() {
        if (this.appointmentDateTime == null || this.status == null) {
            return false;
        }

        // Only future and not yet completed/cancelled appointments are cancellable
        return (this.status == AppointmentStatus.SCHEDULED || this.status == AppointmentStatus.CONFIRMED)
                && this.appointmentDateTime.isAfter(LocalDateTime.now().plusHours(2));
    }


    public String getFormattedDateTime() {
        return appointmentDateTime != null ? appointmentDateTime.toString() : "";
    }
}
